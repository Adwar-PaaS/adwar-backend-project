...
export class CacheInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (!cacheKey) {
      console.warn('[Cache] Skipping cache: empty key');
      return next.handle();
    }
      ...
      tap(async () => {
        for (const key of keys) {
          ...
          try {
            ...
            console.error('[Cache] Failed to resolve key from function:', err);
          }

          if (!resolvedKey || resolvedKey.trim().length === 0) {
            console.warn('[Cache] Skipping invalid cache key:', key);
            continue;
          }
